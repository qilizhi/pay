...
import com.puxun.service.XingPayService;
 ...
 * 七星云支付
 */
public class XingPayAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(XingPayAction.class);
	...
	public void doPay() {
		...
		logger.info("【七星云支付】请求支付参数, request:{"
		...
		String p5_Pid = StringUtils
				.defaultEmpty(request.getParameter("p5_Pid"));// 商品名称
		String p6_Pcat = StringUtils.defaultEmpty(request
				.getParameter("p6_Pcat"));// 商品种类
		String p7_Pdesc = StringUtils.defaultEmpty(request
				.getParameter("p7_Pdesc"));// 商品描述
	//	String p9_SAF = "0";// 送货地址 否 Max(1) 为“1”:
							// 需要用户将送货地址留在易宝支付系统;为“0”:不需要，默认为 ”0”. 10
		String pa_MP = StringUtils.defaultEmpty(request.getParameter("pa_MP"));// 商户扩展信息
																				// 否
																				// Max(200)

		String pd_FrpId = StringUtils.defaultEmpty(request
				.getParameter("pd_FrpId"));// 支付通道编码,到易宝支付网关，易宝支付网关默认显示已开通的全部支付通道.
		String pr_NeedResponse = "1"; // 应答机制 否 Max(1) 固定值为“1”:
										// 需要应答机制;收到易宝支付服务器点对点支付成功通知，必须回写以”success”（无关大小写）开头的字符串，即使您收到成功通知时发现该订单已经处理过，也要正确回写”success”，否则易宝支付将认为您的系统没有收到通知，启动重发机制，直到收到”success”为止。
		...
		String postData = XingPayService.buildPayParmar(parmar);

		String requestPostUrl = ConfigUtil.getXingPayUrl() + postData;
		logger.info("【七星云支付】请求支付URL, request url:{" + requestPostUrl + "}");
	...
	public void callback() throws IOException {
		...
		logger.info("【七星云支付】通知中转平台充值回调host:"+remotehost+" body:"	+ reqcontent+ "");
		logger.info("【七星云支付】通知中转平台充值回调结果, params:{"+ JSONObject.fromObject(request.getParameterMap()).toString()+ "}");
		...
		postData.append("p1_MerId=" + p1_MerId);
		...
		postData.append("&rb_BankId=" + rb_BankId);
		postData.append("&ro_BankOrderId=" + ro_BankOrderId);
		postData.append("&rq_CardNo=" + rq_CardNo);
		postData.append("&ru_Trxtime=" + ru_Trxtime);
		if (StringUtils.isEmpty(hmac)) {
			writeBack("error");
			logger.error("【七星云支付】通知中转平台充值回调结果, hmac(检验码)不能为空");
			return;
		}
	
		postData.append("&hmac=" + hmac);

		logger.info("【七星云支付】回调的参数结果, response:{" + postData.toString() + "}");
		String response = DataHelper.RequestPostUrl(
				ConfigUtil.getXingCallBackUrl(), postData.toString());
		if("success".equalsIgnoreCase(response)){
		writeBack("SUCCESS");// 写成功通知回去
		}else{
			writeBack("ERROR");// 写成功通知回去
		}
		logger.info("【七星云支付】现金平台业务-->中转平台 返回结果, response:{" + response + "}");
		System.out.println("【七星云支付】现金平台业务-->中转平台 返回结果, response:{" + response + "}");
	}
	...
	public static void test() {
		...
		String nhmac = DigestUtil.hmacSign(sbOld,
				"413f2ee1358544179be9265e3cb1780d"); // 数据签名
	...
	public static void main(String[] args) {
		...
		Map<String, String> parmar = new HashMap<String, String>();
		parmar.put("p8_Url",
				"http://192.168.4.65:8085/Pay/puxunCallback.action");
		parmar.put("pa_MP",
				"http://192.168.3.95:8080/cash/paynotity/puxunNotice.action");
		...
		parmar.put("p9_SAF", "0");
		parmar.put("pr_NeedResponse", "1");// 应答机制 否 Max(1) 固定值为“1”:
											// 需要应答机制必须回写以”success”（无关大小写）开头的字符串;

		String postData =XingPayService.buildPayParmar(parmar);

		String requestPostUrl = payUrl + postData;
		System.out.println("dgfsadfg:"+parmar.toString().getBytes());
		String t=HttpUtil.post(parmar.toString().getBytes(),"http://localhost:8085/Pay/xingpayCallback.action");
